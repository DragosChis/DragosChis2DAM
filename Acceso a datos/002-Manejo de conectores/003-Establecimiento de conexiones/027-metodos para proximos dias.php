        ...
        $this->conexion = mysqli_connect(						// Establecemos la conexión a la base de datos
			...
			public function seleccionaTabla($tabla){				// Método para seleccionar registros de una tabla
				...
				while ($row = mysqli_fetch_assoc($result)) {	// Iteramos sobre cada fila del resultado
						$fila = [];													// Creamos un array vacío para almacenar la fila
						foreach($row as $clave=>$valor){			// Iteramos sobre cada columna de la fila
							$fila[$clave] = $valor;						// Agregamos el valor a la fila sin codificación
						}
						$resultado[] = $fila;									// Agregamos la fila al array de resultados
				}
			...
			public function actualizaTabla($tabla, $valores){		// Método para actualizar registros en una tabla
				// Implementación pendiente
			}
			
			public function eliminaTabla($tabla, $id){				// Método para eliminar registros de una tabla
				// Implementación pendiente
			}
